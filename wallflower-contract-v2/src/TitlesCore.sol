...
contract TitlesCore is OwnableRoles, Initializable, UUPSUpgradeable, Receiver {
    ...
    function initialize(address feeReceiver_, address splitFactory_) external initializer {
        ...
        feeManager = new FeeManager(feeReceiver_, splitFactory_);
    ...
    /// @notice Sets the fee strategy for the given {Edition} and `tokenId`.
    /// @param edition_ The {Edition} to set the fee strategy for.
    /// @param tokenId_ The token ID to set the fee strategy for.
    /// @param strategy_ The new fee strategy for the {Edition} and `tokenId`.
    /// @dev Only the owner of the {Edition} can call this function.
    /// @dev Note that this function does NOT modify the fee route. Any change to `revshareBps` will be ignored.
    function setFeeStrategy(address edition_, uint256 tokenId_, Strategy calldata strategy_)
        external
    {
        Edition edition = Edition(edition_);
        if (msg.sender != edition.owner()) revert Unauthorized();

        Strategy memory newStrategy = feeManager.validateStrategy(strategy_);
        edition.setFeeStrategy(tokenId_, newStrategy);

        emit FeeStrategyUpdated(edition_, tokenId_, newStrategy);
    }
    ...
    function _authorizeUpgrade(address newImplementation) internal override onlyOwnerOrRoles(ADMIN_ROLE) {}
