# FeeManager
[Git Source](https://github.com/titlesnyc/wallflower-contract-v2/blob/3def97b53d8f2e1ca0a59e2027614383ba598af9/src/fees/FeeManager.sol)
...
```solidity
uint16 public constant MAX_PROTOCOL_FEE_BPS = 3_333;
```
...
### MAX_ROYALTY_BPS
*The maximum royalty fee in basis points (BPS). Equivalent to 50%.*


```solidity
uint16 public constant MAX_ROYALTY_BPS = 5000;
```


### MIN_ROYALTY_BPS
*The minimum royalty fee in basis points (BPS). Equivalent to 25%.*


```solidity
uint16 public constant MIN_ROYALTY_BPS = 2500;
```
...
```solidity
constructor(address protocolFeeReceiver_, address splitFactory_);
```
**Parameters**
...
function createRoute(
    ...
    address referrer_
) external onlyOwner returns (Target memory receiver);
```
**Parameters**
...
*This function is only callable by the owner (i.e. the {TitlesCore} contract)*
...
### InsufficientFunds
Thrown when an insufficient funds error occurs (i.e. when the amount received is less than the expected fee).
