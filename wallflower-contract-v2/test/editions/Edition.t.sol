...
import {Comment, Node, NodeType, NotOpen, Metadata, Strategy, Target, Unauthorized} from "src/shared/Common.sol";
...
contract EditionTest is Test {
    ...
    function setUp() public {
        edition = new Edition();
        feeManager = new FeeManager(address(0xc0ffee), address(new MockSplitFactory()));
    ...
    function test_mint_timeframe() public {
        vm.prank(address(1));
        edition.setTimeframe(1, uint64(block.timestamp + 10 minutes), uint64(block.timestamp + 2 hours));
    ...
    function test_setMetadata() public {
        Metadata memory metadata = Metadata({label: "New Label", uri: "ipfs.io/new-label", data: new bytes(0)});

        edition.setMetadata(1, metadata);
        assertEq(edition.metadata(1).label, metadata.label);
        assertEq(edition.metadata(1).uri, metadata.uri);

        // Only the manager can set metadata
        vm.prank(address(1));
        vm.expectRevert(Unauthorized.selector);
        edition.setMetadata(1, metadata);
    }
